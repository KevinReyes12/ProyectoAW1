const expresiones = {
    ...
    var pass = document.getElementById("contras");

    const accion = document.getElementById("regist");
    var estado = false;
    var estado2 = false;
    accion.addEventListener("click", (e)=>{
    e.preventDefault();

    if(user.value == "" ||correo.value == "" ||pass.value == ""){
        alert("los campos no pueden estar vacios");
        estado = false;
    }else{
        estado = true;
        if(expresiones.usuario.test(user.value)== false|| expresiones.correo.test(correo.value)== false
        || expresiones.password.test(pass.value)== false){
